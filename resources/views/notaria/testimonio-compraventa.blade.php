...
<style>
    ...
    .separador { text-align: right; color: #000; margin-bottom: 4pt; font-size: 7pt; }
    .encabezado { text-align: center; margin-bottom: 10pt; }
    .encabezado-linea { display: flex; justify-content: space-between; margin-bottom: 4pt; font-size: 8pt; }
    .titulo-seccion { font-weight: bold; text-align: center; margin: 10pt 0 6pt; text-decoration: underline; }
    ...
    .constancia { margin-top: 12pt; border-top: 1px solid #000; padding-top: 6pt; }
</style>
</head>
<body>
<div class="container">
...
<div class="encabezado-linea">
    <span><strong>PRIMER TESTIMONIO</strong></span>
    <span></span>
</div>
<div class="encabezado-linea">
    <span><strong>INSTRUMENTO N°:</strong> {{ $d['num_instrumento'] ?? '______' }}.- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =</span>
</div>
<div class="encabezado-linea">
    <span><strong>MINUTA N°:</strong> {{ $d['num_minuta'] ?? '______' }}.- = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =</span>
</div>
<div class="encabezado-linea">
    <span><strong>COMPRAVENTA DE BIEN FUTURO</strong></span>
</div>
<div class="encabezado-linea">
    <span><strong>VENDEDOR:</strong> {{ strtoupper($d['vendedor_razon_social'] ?? $d['vendedor_nombre'] ?? '') }} -  = = = = = = = = = = = = = = = = = = = = = = = = = =</span>
</div>
<div class="encabezado-linea">
    <span><strong>COMPRADOR(A):</strong> {{ strtoupper($d['comprador_nombre'] ?? '') }}.- = = = = = = = = = = = = = = = = = = = = = = = =</span>
</div>

<p class="parrafo" style="border-top:1px solid #000; margin-top:8pt; padding-top:6pt;"></p>

{{-- INTRODUCCIÓN --}}
<p class="titulo-seccion">INTRODUCCIÓN:</p>

<p class="parrafo">
EN EL DISTRITO, PROVINCIA Y DEPARTAMENTO DE {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }},
A LOS <strong>{{ strtoupper($d['fecha_letras'] ?? '______') }}</strong>;
ANTE MÍ, <strong>{{ strtoupper($empresa->minuta_notario_nombre ?? $empresa->razon_social) }}</strong>,
ABOGADO-NOTARIO DE {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }},
CON DOCUMENTO NACIONAL DE IDENTIDAD Nº {{ $empresa->ruc ?? '________' }},
SUFRAGANTE, NOMBRADO CON RESOLUCIÓN MINISTERIAL NRO. {{ $d['resolucion_ministerial'] ?? '____' }} DE FECHA {{ $d['fecha_resolucion'] ?? '______' }},
CON REGISTRO Nº {{ $d['registro_notario'] ?? '____' }} DEL COLEGIO DE NOTARIOS DE {{ strtoupper($d['colegio_notarios'] ?? 'HUÁNUCO Y PASCO') }},
<strong>COMPARECEN</strong>: = = = = = = = = = = = = =
</p>

@if(isset($d['vendedor_tipo']) && $d['vendedor_tipo'] === 'empresa')
<p class="parrafo">
- <strong>{{ strtoupper($d['vendedor_razon_social']) }}</strong>, CON RUC N° <strong>{{ $d['vendedor_ruc'] }}</strong>
CON DOMICILIO EN {{ strtoupper($d['vendedor_domicilio']) }},
DEBIDAMENTE REPRESENTADO POR SU {{ strtoupper($d['representante_cargo'] ?? 'GERENTE GENERAL') }}
DON <strong>{{ strtoupper($d['representante_nombre']) }}</strong>,
PERUANO, IDENTIFICADO CON DNI Nº <strong>{{ $d['representante_dni'] }}</strong>,
{{ strtoupper($d['representante_estado_civil'] ?? 'SOLTERO') }},
{{ strtoupper($d['representante_profesion'] ?? '________') }},
CON DOMICILIO EN {{ strtoupper($d['representante_domicilio']) }},
CUYAS FACULTADES CORREN INSCRITAS EN LA PARTIDA ELECTRÓNICA Nº <strong>{{ $d['vendedor_partida_registral'] ?? '________' }}</strong>,
DEL REGISTRO DE PERSONAS JURÍDICAS DE LA OFICINA REGISTRAL DE {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }};
QUIEN PROCEDE POR SU PROPIO DERECHO, A QUIEN IDENTIFICO DE LO QUE DOY FE. - = = = = = = = = = = = = = = = = = = = = =
</p>
@else
<p class="parrafo">
- DON/DOÑA <strong>{{ strtoupper($d['vendedor_nombre'] ?? '') }}</strong>,
PERUANO(A), IDENTIFICADO(A) CON DNI N° <strong>{{ $d['vendedor_dni'] ?? '' }}</strong>,
{{ strtoupper($d['vendedor_estado_civil'] ?? 'SOLTERO') }},
CON DOMICILIO EN {{ strtoupper($d['vendedor_domicilio'] ?? '') }};
QUIEN PROCEDE POR SU PROPIO DERECHO, A QUIEN IDENTIFICO DE LO QUE DOY FE. - = = = = = = = = = = = = = = = = = = = = =
</p>
@endif
...
<p class="parrafo">
- DOÑA/DON <strong>{{ strtoupper($d['comprador_nombre']) }}</strong>,
PERUANO(A), IDENTIFICADO(A) CON DNI N° <strong>{{ $d['comprador_dni'] }}</strong>,
{{ strtoupper($d['comprador_estado_civil'] ?? 'SOLTERO') }},
{{ strtoupper($d['comprador_profesion'] ?? '________') }},
CON DOMICILIO EN {{ strtoupper($d['comprador_domicilio']) }},
DE TRÁNSITO POR ESTA CIUDAD; QUIEN PROCEDE POR SU PROPIO DERECHO, A QUIEN IDENTIFICO DE LO QUE DOY FE. - = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
</p>

<p class="parrafo">
LOS COMPARECIENTES SON MAYORES DE EDAD, SUFRAGANTES, HÁBILES PARA CONTRATAR E INTELIGENTES EN EL IDIOMA CASTELLANO,
QUIENES SE OBLIGAN CON CAPACIDAD, LIBERTAD Y CONOCIMIENTO SUFICIENTE, DE LO QUE DOY FE.
ASÍ COMO DE HABER CONSTATADO QUE PROCEDEN CON CAPACIDAD, LIBERTAD Y CONOCIMIENTO CON QUE SE OBLIGA CONFORME A LA LEY DEL NOTARIADO,
ASIMISMO SE ADVIRTIÓ SOBRE LOS EFECTOS LEGALES DEL PRESENTE INSTRUMENTO PÚBLICO NOTARIAL,
DE CONFORMIDAD AL ARTÍCULO VEINTISIETE DEL DECRETO LEGISLATIVO NÚMERO MIL CUARENTA Y NUEVE
Y ME ENTREGAN UNA MINUTA DE <strong>COMPRAVENTA DE BIEN FUTURO</strong>,
PARA QUE SU TENOR SE ELEVE A ESCRITURA PÚBLICA LA CUAL ARCHIVO EN SU LEGAJO RESPECTIVO,
BAJO EL NÚMERO RESPECTIVO, SIENDO SU CONTENIDO LITERAL COMO SIGUE: = = = = = = = = =
</p>

{{-- MINUTA --}}
<p class="titulo-seccion">MINUTA.-</p>

<p class="parrafo">
<strong>SEÑOR NOTARIO:</strong> SÍRVASE UD. EXTENDER EN SU REGISTRO DE ESCRITURAS PÚBLICAS UNA DE
<strong>COMPRAVENTA DE BIEN FUTURO</strong> QUE CELEBRAN DE UNA PARTE
@if(isset($d['vendedor_tipo']) && $d['vendedor_tipo'] === 'empresa')
LA SOCIEDAD DENOMINADA <strong>{{ strtoupper($d['vendedor_razon_social']) }}</strong>,
CON RUC N° <strong>{{ $d['vendedor_ruc'] }}</strong>,
CON DOMICILIO EN EL {{ strtoupper($d['vendedor_domicilio']) }},
INSCRITO EN LA PARTIDA REGISTRAL N° <strong>{{ $d['vendedor_partida_registral'] ?? '________' }}</strong> DEL REGISTRO DE PERSONAS JURÍDICAS DE {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }},
DEBIDAMENTE REPRESENTADO POR SU {{ strtoupper($d['representante_cargo'] ?? 'GERENTE GENERAL') }}
<strong>{{ strtoupper($d['representante_nombre']) }}</strong>,
DE NACIONALIDAD PERUANO, IDENTIFICADO CON DOCUMENTO NACIONAL DE IDENTIDAD N° <strong>{{ $d['representante_dni'] }}</strong>,
DE ESTADO CIVIL {{ strtoupper($d['representante_estado_civil'] ?? 'SOLTERO') }},
DE PROFESIÓN {{ strtoupper($d['representante_profesion'] ?? '________') }},
CON DOMICILIO EN {{ strtoupper($d['representante_domicilio']) }},
A QUIEN EN ADELANTE SE LE DENOMINARÁ <strong>EL VENDEDOR</strong>
@else
<strong>{{ strtoupper($d['vendedor_nombre'] ?? '') }}</strong>,
DE NACIONALIDAD PERUANA, IDENTIFICADO CON DOCUMENTO NACIONAL DE IDENTIDAD N° <strong>{{ $d['vendedor_dni'] ?? '' }}</strong>,
DE ESTADO CIVIL {{ strtoupper($d['vendedor_estado_civil'] ?? 'SOLTERO') }},
CON DOMICILIO EN {{ strtoupper($d['vendedor_domicilio'] ?? '') }},
A QUIEN EN ADELANTE SE LE DENOMINARÁ <strong>EL VENDEDOR</strong>
@endif
Y, DE OTRA PARTE, <strong>{{ strtoupper($d['comprador_nombre']) }}</strong>,
DE NACIONALIDAD PERUANA, IDENTIFICADO CON DOCUMENTO NACIONAL DE IDENTIDAD N° <strong>{{ $d['comprador_dni'] }}</strong>,
DE ESTADO CIVIL {{ strtoupper($d['comprador_estado_civil'] ?? 'SOLTERO') }},
DE PROFESIÓN {{ strtoupper($d['comprador_profesion'] ?? '________') }},
CON DOMICILIO EN {{ strtoupper($d['comprador_domicilio']) }},
DE TRÁNSITO POR ESTA CIUDAD,
A QUIEN EN ADELANTE SE LE DENOMINARÁ <strong>LA COMPRADORA</strong>;
EN LOS TÉRMINOS Y CONDICIONES DE LAS CLÁUSULAS SIGUIENTES:
</p>

<p class="parrafo"><strong>PRIMERA.-</strong> <strong>EL VENDEDOR</strong> ES COPROPIETARIO DEL PREDIO SIGNADO COMO LOTE Nº 1 EN EL PLANO DE LOTIZACIÓN DEL FUNDO RÚSTICO ESPERANZA, DEL DISTRITO DE AMARILIS, PROVINCIA Y DEPARTAMENTO DE {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }}, CON PARTIDA REGISTRAL N° <strong>{{ $d['predio_partida'] }}</strong> DE LA OFICINA REGISTRAL DE {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }}, EN LA QUE CONSTA SUS DEMÁS CARACTERÍSTICAS FÍSICAS. = = = = = = = = = = = = =</p>
...
<p class="parrafo"><strong>QUINTA.-</strong> LAS PARTES, DE COMÚN ACUERDO, ESTABLECEN QUE EL PRECIO TOTAL DE VENTA POR EL PREDIO MATERIA DE ESTA MINUTA ES DE <strong>S/ {{ number_format(floatval($d['precio_total']), 2) }} ({{ strtoupper($d['precio_total_letras'] ?? '') }})</strong> TOTALMENTE CANCELADO POR <strong>LA COMPRADORA</strong> MEDIANTE {{ strtoupper($d['forma_pago_detalle'] ?? '') }}. EN TAL SENTIDO <strong>EL VENDEDOR</strong> DECLARA TOTALMENTE CANCELADO EL PRECIO DE VENTA. = = = = =</p>
...
<p class="parrafo"><strong>OCTAVA.-</strong> <strong>EL VENDEDOR</strong> SE OBLIGA A OBTENER LA AUTORIZACIÓN MUNICIPAL SEÑALADA EN LA CLÁUSULA QUE PRECEDE DENTRO DEL PLAZO MÁXIMO DE {{ strtoupper($d['plazo_anos'] ?? 'TRES') }} AÑOS, CONTADOS A PARTIR DE LA FECHA DE ESTA MINUTA. PARA TAL FIN, <strong>LA COMPRADORA</strong> DEBERÁ REALIZAR TODAS LAS ACTIVIDADES NECESARIAS A SU CARGO POR ANTE LA {{ strtoupper($d['proyecto_municipalidad'] ?? 'MUNICIPALIDAD DISTRITAL DE AMARILIS') }} – {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }}. = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =</p>
...
<p class="parrafo"><strong>DÉCIMO.-</strong> UNA VEZ QUE SE HAYA CUMPLIDO LA CONDICIÓN SUSPENSIVA, <strong>LA COMPRADORA</strong> DEBERÁ DECLARAR LA TRANSFERENCIA CONTENIDA EN ESTE DOCUMENTO POR ANTE LA {{ strtoupper($d['proyecto_municipalidad'] ?? 'MUNICIPALIDAD DISTRITAL DE AMARILIS') }} – {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }}, PARA LOS FINES DEL PAGO DEL IMPUESTO PREDIAL Y DEMÁS IMPUESTOS QUE AFECTEN AL PREDIO SUB MATERIA, ASÍ COMO INSCRIBIR ESTA COMPRA VENTA ANTE EL REGISTRO DE PREDIOS DE LA OFICINA REGISTRAL DE {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }}.</p>
...
<p class="parrafo"><strong>DÉCIMO TERCERA.-</strong> EN EL SUPUESTO QUE UNA DE LAS PARTES NO CUMPLA CON LAS OBLIGACIONES QUE SE DERIVAN DE ESTE CONTRATO, LA OTRA PARTE PODRÁ, ALTERNATIVAMENTE, EXIGIR EL CUMPLIMIENTO DE LA OBLIGACIÓN DE SU CONTRAPARTE EN LOS TÉRMINOS CONTENIDOS EN ESTE CONTRATO O RESOLVER EL CONTRATO. EN ESTE ÚLTIMO SUPUESTO, LA PARTE QUE DESEA RESOLVERLA CURSARÁ PREVIAMENTE COMUNICACIÓN POR CONDUCTO NOTARIAL A LA OTRA PARTE, REQUIRIENDO EL CUMPLIMIENTO DE LA OBLIGACIÓN INCUMPLIDA EN EL PLAZO DE 15 DÍAS DE RECIBIDA DICHA COMUNICACIÓN. SI PERSISTIERA DICHO INCUMPLIMIENTO DARÁ POR RESUELTO ESTE CONTRATO.</p>
...
<p class="parrafo"><strong>DÉCIMA CUARTA.-</strong> LAS PARTES ACUERDAN QUE TODOS LOS GASTOS Y TRIBUTOS QUE SE ORIGINEN A LA CELEBRACIÓN, FORMALIZACIÓN Y EJECUCIÓN DEL PRESENTE CONTRATO SERÁN ASUMIDOS <strong>LA COMPRADORA</strong>. = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =</p>
...
<p class="parrafo"><strong>DÉCIMO SEXTA.-</strong> PARA LA VALIDEZ DE TODAS LAS COMUNICACIONES Y NOTIFICACIONES A LAS PARTES, CON MOTIVO DE LA EJECUCIÓN DE ESTE CONTRATO, AMBAS SEÑALAN COMO SUS RESPECTIVOS DOMICILIOS LOS INDICADOS EN LA INTRODUCCIÓN DE ESTE DOCUMENTO. EL CAMBIO DE DOMICILIO DE CUALQUIERA DE LAS PARTES SURTIRÁ EFECTO DESDE LA FECHA DE COMUNICACIÓN DE DICHO CAMBIO A LA OTRA PARTE, POR VÍA NOTARIAL. = = = = = = = = = = = = = = = = = = =</p>

<p class="parrafo"><strong>DÉCIMO SÉPTIMA.-</strong> EN TODO LO NO PREVISTO POR LAS PARTES EN EL PRESENTE CONTRATO, AMBAS SE SOMETEN A LO ESTABLECIDO POR LAS NORMAS DEL CÓDIGO CIVIL Y DEMÁS DEL SISTEMA JURÍDICO QUE RESULTEN APLICABLES. = = = = = = = = = = = = = = = = = = = = = = = = = = =</p>

<p class="parrafo">SÍRVASE UD. SEÑOR NOTARIO, AGREGAR LO QUE FUERA DE LEY. - {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }}, {{ $d['fecha_minuta'] ?? '' }}.</p>

<p class="parrafo"><strong>FIRMAS Y HUELLAS DACTILARES DE:</strong>
{{ strtoupper($d['vendedor_razon_social'] ?? $d['vendedor_nombre'] ?? '') }} -
{{ strtoupper($d['representante_nombre'] ?? '') }}, VENDEDOR. -
{{ strtoupper($d['comprador_nombre']) }}, COMPRADORA. -</p>
...
<p class="parrafo"><strong>FIRMADO:</strong> {{ strtoupper($empresa->minuta_notario_nombre ?? $empresa->razon_social) }}. - ABOGADO - NOTARIO. - SELLO NOTARIAL. –</p>
</div>
...
<p class="parrafo"><strong>FIRMAS Y HUELLAS DACTILARES DE:</strong>
{{ strtoupper($d['vendedor_razon_social'] ?? $d['vendedor_nombre'] ?? '') }} -
{{ strtoupper($d['representante_nombre'] ?? '') }}, FIRMÓ EL: {{ $d['fecha_firma'] ?? '______' }}. -
{{ strtoupper($d['comprador_nombre']) }}, FIRMÓ EL: {{ $d['fecha_firma'] ?? '______' }}. -</p>
...
<p class="parrafo"><strong>FIRMADO:</strong> {{ strtoupper($empresa->minuta_notario_nombre ?? $empresa->razon_social) }}, ABOGADO - NOTARIO PÚBLICO DE {{ strtoupper($d['ciudad'] ?? 'HUÁNUCO') }}. UN SELLO NOTARIAL. -</p>
</div>
