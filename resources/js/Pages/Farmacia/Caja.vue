        ...
        <Teleport to="body">
            <div v-if="modalCierre" style="position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:1000; display:flex; align-items:center; justify-content:center;">
                <div style="background:white; border-radius:20px; padding:28px; width:100%; max-width:500px; max-height:90vh; overflow-y:auto; box-shadow:0 20px 60px rgba(0,0,0,0.2);">
                    ...
                    <!-- Diferencia -->
                    <div v-if="formCierre.monto_final !== null && formCierre.monto_final !== ''" :style="{
...
const formCierre   = ref({ monto_final: null, observaciones: '' })
...
const diferenciaCierre = computed(() => {
    if (formCierre.value.monto_final === null || formCierre.value.monto_final === '' || !props.caja_abierta) return 0
...
const guardarCorreccion = () => {
    ...
    if (formCorreccion.value.monto_inicial_nuevo === null || formCorreccion.value.monto_inicial_nuevo === '') return
