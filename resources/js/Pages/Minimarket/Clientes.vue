<template>
    <AppLayout title="Clientes" subtitle="Gestión de clientes minimarket">

        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
            <input v-model="busqueda" placeholder="🔍 Buscar por nombre o documento..."
        ...
        <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:16px; margin-bottom:24px;">
            ...
            <div style="background:white; border-radius:12px; padding:16px; border:1px solid #E2E8F0;">
                ...
                <p style="font-size:24px; font-weight:700; color:#14B8A6; margin:0;">{{ clientes.filter(c=>c.tipo_documento==='RUC').length }}</p>
            </div>
            <div style="background:white; border-radius:12px; padding:16px; border:1px solid #E2E8F0;">
                <p style="font-size:12px; color:#94A3B8; margin:0 0 4px;">Con DNI</p>
                <p style="font-size:24px; font-weight:700; color:#8B5CF6; margin:0;">{{ clientes.filter(c=>c.tipo_documento==='DNI').length }}</p>
            </div>
        </div>
            ...
            <table style="width:100%; border-collapse:collapse;">
                ...
                <tbody>
                    ...
                    <tr v-for="c in clientesFiltrados" :key="c.id"
                        ...
                        <td style="padding:14px 20px;">
                            <span style="padding:2px 8px; border-radius:20px; font-size:11px; font-weight:700; background:#EFF6FF; color:#1D4ED8;">{{ c.tipo_documento }}</span>
                            <p style="font-size:13px; color:#475569; margin:4px 0 0;">{{ c.numero_documento }}</p>
                        </td>
                        <td style="padding:14px 20px;">
                            <p v-if="c.celular" style="font-size:12px; color:#94A3B8; margin:0;">📞 {{ c.celular }}</p>
                        ...
                        <td style="padding:14px 20px; font-size:13px; color:#475569;">
                            <p style="margin:0;">{{ c.direccion || '—' }}</p>
                            <p v-if="c.distrito" style="font-size:12px; color:#94A3B8; margin:2px 0 0;">{{ c.distrito }}</p>
                        </td>
        ...
        <div v-if="modal" style="position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:1000; display:flex; align-items:flex-start; justify-content:center; overflow-y:auto; padding:12px;">
            <div style="background:white; border-radius:16px; padding:28px; width:100%; max-width:560px; margin:auto;">
...
const guardar = () => {
    ...
    if (editando.value) {
        router.put(`/minimarket/clientes/${editando.value.id}`, form.value, {
            onSuccess: () => { modal.value = false; procesando.value = false },
            onError:   () => { procesando.value = false }
        })
    } else {
        router.post('/minimarket/clientes', form.value, {
            onSuccess: () => { modal.value = false; procesando.value = false },
            onError:   () => { procesando.value = false }
        })
    }
...
const eliminar = (c) => {
    if (confirm(`¿Eliminar cliente "${c.razon_social}"?`)) {
        router.delete(`/minimarket/clientes/${c.id}`)
    }
}
</script>
