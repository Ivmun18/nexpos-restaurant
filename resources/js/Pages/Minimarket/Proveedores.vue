<template>
    <AppLayout title="Proveedores" subtitle="Gestión de proveedores minimarket">
                ...
                <tbody>
                    ...
                    <tr v-for="p in proveedoresFiltrados" :key="p.id"
                        ...
                        <td style="padding:14px 20px;">
                            ...
                            <span v-if="p.agente_retencion" style="font-size:11px; padding:2px 8px; background:#FEF3C7; color:#92400E; border-radius:20px; font-weight:600;">Ag. Retención</span>
                        </td>
                        <td style="padding:14px 20px;">
                        ...
                        <td style="padding:14px 20px; font-size:13px; color:#475569;">
                            ...
                            <p v-if="p.distrito" style="font-size:12px; color:#94A3B8; margin:2px 0 0;">{{ p.distrito }}</p>
                        </td>
...
const guardar = () => {
    ...
    if (editando.value) {
        router.put(`/minimarket/proveedores/${editando.value.id}`, form.value, {
            onSuccess: () => { modal.value = false; procesando.value = false },
            onError:   () => { procesando.value = false }
        })
    } else {
        router.post('/minimarket/proveedores', form.value, {
            onSuccess: () => { modal.value = false; procesando.value = false },
            onError:   () => { procesando.value = false }
        })
    }
...
const eliminar = (p) => {
    if (confirm(`¿Eliminar proveedor "${p.razon_social}"?`)) {
        router.delete(`/minimarket/proveedores/${p.id}`)
    }
}
</script>
