<template>
    <AppLayout title="Cajero" subtitle="Ventas pendientes de cobro">

        <!-- Stats caja -->
        <div style="background:linear-gradient(135deg,#14B8A6,#0F766E); border-radius:16px; padding:20px; margin-bottom:24px; color:white;">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    <p style="font-size:13px; opacity:0.8; margin:0 0 4px;">Caja abierta</p>
                    <p style="font-size:20px; font-weight:800; margin:0;">💰 S/ {{ Number(caja_abierta.monto_inicial).toFixed(2) }} inicial</p>
                </div>
                <div style="text-align:right;">
                    <p style="font-size:13px; opacity:0.8; margin:0 0 4px;">Pendientes</p>
                    <p style="font-size:32px; font-weight:900; margin:0;">{{ ventas_pendientes.length }}</p>
                </div>
            </div>
        </div>

        <!-- Sin pendientes -->
        <div v-if="!ventas_pendientes.length" style="text-align:center; padding:60px 20px; background:white; border-radius:16px; border:1px solid #E2E8F0;">
...
const cobrar = (venta) => {
    ...
    router.post(`/minimarket/cajero/${venta.id}/cobrar`, {
