<template>
    <AppLayout title="Ventas" subtitle="Historial de ventas de ferretería">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px;">
            ...
            <a href="/ferreteria/pos" style="padding:10px 20px; background:linear-gradient(135deg,#14B8A6,#0F766E); color:white; border-radius:10px; font-size:14px; font-weight:600; text-decoration:none;">
                + Nueva Venta
            </a>
        </div>
            ...
            <table style="width:100%; border-collapse:collapse;">
                ...
                <tbody>
                    ...
                    <tr v-for="venta in ventas.data" :key="venta.id"
                        :style="`border-top:1px solid #F1F5F9; transition:background 0.15s; ${venta.estado === 'anulado' ? 'opacity:0.5; background:#FFF5F5;' : ''}`"
                        @mouseenter="e => e.currentTarget.style.background = venta.estado === 'anulado' ? '#FFF5F5' : '#F8FAFC'"
                        @mouseleave="e => e.currentTarget.style.background = venta.estado === 'anulado' ? '#FFF5F5' : 'white'">
                        ...
                        <td style="padding:14px 20px; text-align:center;">
                            <div style="display:flex; gap:6px; justify-content:center; flex-wrap:wrap;">
                                <a :href="`/ferreteria/ventas/${venta.id}`"
        ...
        <div v-if="ventas.last_page > 1" style="display:flex; justify-content:center; gap:8px; margin-top:20px;">
            <a v-for="page in ventas.last_page" :key="page"
                :href="`/ferreteria/ventas?page=${page}`"
                :style="{
                    padding: '8px 14px',
                    borderRadius: '8px',
                    fontSize: '14px',
...
const reintentar = (id) => {
    if (confirm('¿Reintentar envío a SUNAT?')) {
        router.post(`/ferreteria/ventas/${id}/reintentar`)
    }
}
...
const anular = (id) => {
    if (confirm('¿Anular este comprobante en SUNAT?')) {
        router.post(`/ferreteria/ventas/${id}/anular`)
    }
}
