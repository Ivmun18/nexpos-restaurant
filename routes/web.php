...
Route::get('/dashboard', function () {
    ...
    if ($industryType === 'notaria') {
        ...
        // Top actos notariales por tipo
        $topActos = \App\Models\ActoNotarial::where('empresa_id', $empresaId)
            ->selectRaw('tipo_acto, COUNT(*) as total_cantidad, SUM(monto_cobrar) as total_monto')
            ->groupBy('tipo_acto')
            ->orderByDesc('total_monto')
            ->limit(5)
            ->get()
            ->map(function($item) {
                ...
                $item->tipo_acto_label = $labels[$item->tipo_acto] ?? ucfirst($item->tipo_acto);
                return $item;
            });

        // Últimos actos registrados
        $ultimosActos = \App\Models\ActoNotarial::where('empresa_id', $empresaId)
            ->with('cliente')
            ...
            ->map(function($acto) {
                ...
                $acto->tipo_acto_label = $labels[$acto->tipo_acto] ?? ucfirst($acto->tipo_acto);
                $acto->monto_total = $acto->monto_cobrar; // Agregar monto_total para el componente Vue
                return $acto;
            });
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/ferreteria/ventas/exportar', [\App\Http\Controllers\Ferreteria\ReportesFerretoriaController::class, 'exportarVentas'])->name('ferreteria.ventas.exportar');
    ...
    Route::post('/ferreteria/ventas/{venta}/reintentar', [\App\Http\Controllers\Ferreteria\ReportesFerretoriaController::class, 'reintentar'])->name('ferreteria.ventas.reintentar');
...
Route::post('/portal-cliente/consultar', [\App\Http\Controllers\Notaria\PortalClienteController::class, 'consultar'])->name('portal.cliente.consultar');
...
Route::middleware(['auth', 'notaria.rol'])->group(function () {
    ...
    Route::post('/notaria/actos/{acto}/partes/reordenar', [App\Http\Controllers\Notaria\ActoParteController::class, 'reordenar'])->name('notaria.partes.reordenar');
});
...
Route::prefix('notaria/indice')->name('notaria.indice.')->middleware(['auth', 'notaria.rol'])->group(function () {
    ...
    Route::post('/cerrar-mes', [App\Http\Controllers\Notaria\IndiceNotarialController::class, 'cerrarMes'])->name('cerrar-mes');
    Route::get('/exportar', [App\Http\Controllers\Notaria\IndiceNotarialController::class, 'exportar'])->name('exportar');
});

// ═══════════════════════════════════════════
// GIMNASIO
// ═══════════════════════════════════════════
Route::middleware(['auth', 'verified'])->prefix('gimnasio')->name('gimnasio.')->group(function () {
    Route::get('/dashboard', [\App\Http\Controllers\Gimnasio\DashboardGimnasioController::class, 'index'])->name('dashboard');

    // Miembros
    Route::get('/miembros',                     [\App\Http\Controllers\Gimnasio\MiembroController::class, 'index'])->name('miembros.index');
    Route::post('/miembros',                    [\App\Http\Controllers\Gimnasio\MiembroController::class, 'store'])->name('miembros.store');
    ...
    Route::delete('/planes/{plan}',     [\App\Http\Controllers\Gimnasio\PlanController::class, 'destroy'])->name('planes.destroy');

    // Instructores
    Route::get('/instructores',                     [\App\Http\Controllers\Gimnasio\InstructorController::class, 'index'])->name('instructores.index');
