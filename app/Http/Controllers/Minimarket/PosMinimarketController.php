    ...
    $items = $venta->detalle->map(function($item) use ($esRus) {
        if ($esRus) {
            return [
                ...
                // APISUNAT — código 20 (Exonerado - Operación Onerosa), no
                // '40' (Inafecto): $esRus incluye zona_exonerada además de
                // RUS real, y para exoneración por zona (Ley 27037) la
                // línea debe declarar el mismo tributo "EXO" que usa la
                // cabecera (cac:TaxTotal más abajo, TaxScheme 9997) — con
                // '40'/'INA' la línea quedaba bajo un tributo distinto al
                // de la cabecera y SUNAT rechazaba con el error 2638
                // ("debe consignar el tag del total del tributo").
                'porcentaje_igv'              => 0,
