    ...
    public function store(Request $request, Mesa $mesa)
{
    ...
    public function anularPlato(Request $request, PedidoDetalle $detalle)
    {
        ...
        // No se puede anular un plato ya pagado
        if ($detalle->pagado) {
            return back()->with('error', 'No se puede anular un plato que ya fue pagado.');
        }

        // No se puede anular si ya estaba anulado
        if ($detalle->anulado) {
            return back()->with('error', 'Este plato ya estaba anulado.');
        }
        ...
        // Recalcular el total del pedido (los anulados no suman)
        $pedido = $detalle->pedido;
        if ($pedido) {
            $nuevoTotal = PedidoDetalle::where('pedido_id', $pedido->id)
                ->where('anulado', false)
                ->sum('subtotal');
            $pedido->update(['total' => $nuevoTotal]);
        }

        return back()->with('success', 'Plato anulado: ' . $detalle->nombre_producto);
    }
